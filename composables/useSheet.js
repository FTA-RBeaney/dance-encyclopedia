let sheetRange = "Registered!A1:D200";

const getVarsWorkshop = () => {
...
export async function allRowsWorkshop() {
  const { SPREAD_SHEET_ID, GOOGLE_API_KEY } = getVarsWorkshop();
...
export async function singleRow(row) {
  ...
  const rowRange = `Registered!A${row}:D${row}`;
